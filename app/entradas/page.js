...
export default function EntradasPage() {
  ...
  async function cargarTodo() {
    const [rProd, rEntradas] = await Promise.all([
      fetch('/api/productos'),
      fetch('/api/entradas'),
    ]);
  ...
  return (
    <main style={{ fontFamily: 'sans-serif', padding: '24px', maxWidth: '700px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h1 style={{ margin: 0 }}>Entrada de mercancía — Kennedy</h1>
        <a href="/">Inicio</a>
      </div>

      <form onSubmit={confirmarEntrada} style={formStyle}>
        ...
        <tbody>
          {entradas.map((en) => (
            <tr key={en.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={td}>
                {new Date(en.creado_en).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })}
              </td>
              <td style={td}>
                {en.referencia} — {en.nombre}
              </td>
              <td style={td}>{en.cantidad}</td>
              <td style={td}>{en.nota || '-'}</td>
            </tr>
          ))}
          {entradas.length === 0 && (
            <tr>
